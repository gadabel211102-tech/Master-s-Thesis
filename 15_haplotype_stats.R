...
ANALYSIS_LABEL_RAW <- trimws(Sys.getenv("TFM_ANALYSIS_LABEL", unset = ""))
ANALYSIS_LABEL <- if (nzchar(ANALYSIS_LABEL_RAW)) gsub("[^A-Za-z0-9_-]", "_", ANALYSIS_LABEL_RAW) else ""
SAMPLE_INCLUDE_FILE <- trimws(Sys.getenv("TFM_SAMPLE_INCLUDE_FILE", unset = ""))
ANALYSIS_CONTEXT <- trimws(Sys.getenv("TFM_ANALYSIS_CONTEXT", unset = ""))
OUTPUT_STEM <- if (nzchar(ANALYSIS_LABEL)) paste0("GSDMB_Haplotype_Results_", ANALYSIS_LABEL) else "GSDMB_Haplotype_Results"

OUT_XLSX  <- file.path(OUT_DIR, paste0(OUTPUT_STEM, ".xlsx"))
MAIN_FIG_DIR <- file.path(OUT_DIR, if (nzchar(ANALYSIS_LABEL)) paste0("main_figures_", ANALYSIS_LABEL) else "main_figures")
SUPP_FIG_DIR <- file.path(OUT_DIR, if (nzchar(ANALYSIS_LABEL)) paste0("supplementary_figures_", ANALYSIS_LABEL) else "supplementary_figures")
...
FDR_THRESHOLD <- 0.10
FDR_LABEL <- sprintf("FDR < %.2f", FDR_THRESHOLD)
RAW_P_LABEL <- sprintf("p < %.2f", RAW_P_THRESHOLD)
...
define_ld_blocks <- function(ld_matrix, snp_meta, threshold = 0.80,
...
run_assoc_for_region <- function(region_name, a1_mat, a2_mat, region_meta,
  ...
  out$Significant_Fisher     <- out$P_Fisher < RAW_P_THRESHOLD
  out$Significant_FDR        <- out$FDR < FDR_THRESHOLD
...
make_forest_plot <- function(assoc_df, region_name, out_file) {
  ...
  plot_df <- assoc_df %>%
    ...
    mutate(
      ...
      sig_label  = case_when(
        FDR < FDR_THRESHOLD       ~ FDR_LABEL,
        P_Fisher < RAW_P_THRESHOLD  ~ RAW_P_LABEL,
        TRUE             ~ "ns"
      ),
  ...
  p <- ggplot(plot_df, aes(x = OR, y = label,
    ...
    scale_colour_manual(values = stats::setNames(c("#D55E00", "#E69F00", "grey55"), c(FDR_LABEL, RAW_P_LABEL, "ns")),
                        name = "Significance") +
    scale_shape_manual(values = stats::setNames(c(18, 17, 16), c(FDR_LABEL, RAW_P_LABEL, "ns")),
                       name = "Significance") +
...
make_association_overview_plot <- function(summary_df, out_file) {
  plot_df <- summary_df %>%
    ...
    mutate(
      ...
      sig_label  = case_when(
        Best_FDR < FDR_THRESHOLD  ~ FDR_LABEL,
        Best_P_Fisher < RAW_P_THRESHOLD ~ RAW_P_LABEL,
        TRUE             ~ "ns"
      ),
  ...
  p <- ggplot(plot_df, aes(x = fct_reorder(Region_short, neg_log_p, .desc = TRUE),
    ...
    scale_colour_manual(
      ...
      name = "Region type") +
    scale_shape_manual(
      values = stats::setNames(c(18, 17, 16), c(FDR_LABEL, RAW_P_LABEL, "ns")),
      name = "Significance") +
...
sample_filter_mode <- "FullStudySet"
if (nzchar(SAMPLE_INCLUDE_FILE)) {
  include_df <- read.table(SAMPLE_INCLUDE_FILE, header = FALSE, sep = "	", stringsAsFactors = FALSE, quote = "", comment.char = "")
  include_samples <- unique(trimws(as.character(include_df[[1]])))
  include_samples <- include_samples[nzchar(include_samples)]
  meta_raw <- meta_raw %>% filter(Sample %in% include_samples)
  sample_filter_mode <- "ExplicitSampleList"
}
...
cat(sprintf("  Sample filter mode: %s\n", sample_filter_mode))
if (nzchar(ANALYSIS_LABEL)) cat(sprintf("  Analysis label: %s\n", ANALYSIS_LABEL))
if (nzchar(ANALYSIS_CONTEXT)) cat(sprintf("  Analysis context: %s\n", ANALYSIS_CONTEXT))
...
cat(sprintf("  - Single BH-FDR applied within each region across all haplotype-comparison tests (threshold %.2f)\n", FDR_THRESHOLD))
