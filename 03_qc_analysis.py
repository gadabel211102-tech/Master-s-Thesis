...
def load_annotation_for_cohort(root_path: str) -> pd.DataFrame:
    ...
    return ann.drop_duplicates(["chr", "start", "end"])
...
def plot_all(full_qc_df, cohort_dfs, annotation_df, all_cov, failing_samples):
    ...
    # Prepare failure set for tick colouring
    clean_failing_samples = {normalise_sample(s) for s in failing_samples}

    # ==========================================================================
    # 02. Cohort heatmaps (force all columns, no autoskip, wide canvas)
    # ==========================================================================
    ...
    for cohort, df in cohort_dfs.items():
        ...
        # DEBUG: prove matrix has expected shape (amplicons × samples)
        print(f"[DEBUG] Heatmap {cohort}: matrix shape = {dmat.shape}")

        # Persist the sample order actually plotted (useful to verify)
        if cohort == "Breast Tumour":
            pd.Series(s_cols).to_csv(
                os.path.join(PATHS["out_dir"], "breast_tumour_heatmap_order.txt"),
                index=False, header=False
            )

        # Build plot matrix and mask
        plotmat = np.log10(dmat.fillna(0) + 1)
        cmap = sns.color_palette("magma", as_cmap=True)
        cmap.set_bad(color="#B0B0B0")

        # Wider figure so 60 columns are clear
        plt.figure(figsize=(20, 8))
        ax = sns.heatmap(plotmat, cmap=cmap, mask=dmat.isna(), vmin=0, vmax=4)

        # Force a tick at every column and draw all labels (no autoskip)
        ...
        # Optional separators so every column is visually distinct
        for x in range(len(s_cols) + 1):
            ax.axvline(x, color="white", lw=0.3, alpha=0.6)

        # Highlight failing samples
    ...
    # Optional: per-sample amplicon counts (Breast Tumour only)
    cohort_name_bt = "Breast Tumour"
    df_bt = cohort_dfs.get(cohort_name_bt)
    if df_bt is not None:
        s_cols_bt = [c for c in df_bt.columns if c not in meta]
        counts = df_bt.set_index("id")[s_cols_bt].notna().sum().sort_index()
        plt.figure(figsize=(20, 4))
        counts.plot(kind="bar", width=0.9)
        plt.title(f"{cohort_name_bt}: amplicons with numeric depth per sample")
        plt.ylabel("Amplicons with depth")
        plt.xticks(rotation=90, fontsize=6)
        plt.tight_layout()
        plt.savefig(f"{PATHS['plots_dir']}/02b_amplicon_counts_{cohort_name_bt.replace(' ', '_')}.png", dpi=300)
        plt.close()

    # ==========================================================================
    # 03. Global Heatmap (wide; modest tick font)
    # ==========================================================================
    ...
    structural_lines = n_cohorts * 6  # seaborn internal structure
    ...
    # ==========================================================================
    # 13. Systemic Amplicon Failures — by cohort (annotation)
    # ==========================================================================
    if cohort_dfs:
        ...
        for ax, cohort in zip(axes, cohorts):
            ...
            agg = (tmp.groupby("annotation", as_index=False)["fail_count"]
            ...
            plot_df = agg.iloc[::-1]
            ax.barh(plot_df["annotation"], plot_df["fail_count"],
...
def run_pipeline():
    ...
    # Export per-sample coverage tables (harmonised format)
    for cohort, df in cohort_dfs.items():
        ...
        for s in s_cols:
            ...
            out.to_csv(out_path, sep="\t", compression="gzip")  # compressed TSV

    # Generate ALL figures (cohort plots, global plots, landscapes, failures, etc.)
