...
def load_bed_annotations(bed_path: str, logger: logging.Logger) -> pd.DataFrame:
    ...
    try:
        ...
        # Create annotation (prefer rs_id if not '.', otherwise use gene)
        if 'gene_snp' in bed_df.columns:
            bed_df['annotation'] = bed_df['gene_snp'].replace('.', np.nan)
            if 'extra' in bed_df.columns:
                bed_df['annotation'] = bed_df['annotation'].fillna(bed_df['extra'])
        else:
            bed_df['annotation'] = bed_df.get('numeric_id', 'unknown')
...
def process_zero_coverage(zero_pattern: str, logger: logging.Logger) -> Optional[pd.DataFrame]:
    ...
    for z_file in all_zero_files:
        ...
        try:
            ...
            # Average zero bases across samples
            ...
            merged_zero = temp if merged_zero is None else pd.merge(
                merged_zero, temp, on='match_id', how='outer'
            )
...
def run_comprehensive_audit(
    ...
    if merged_zero is not None:
        ...
        logger.info(f"Added zero-base metrics")
    ...
    logger.info(f"\n✅ Comprehensive audit saved: {output_file}")
