...
def _add_margin_labels(ax, points, side="right", max_labels=6, fontsize=7.5):
    if not points:
        return
...
def _run_logit(df_model, formula, label_prefix):
    ...
    try:
        ...
        return {
            f"Test_Adj_{label_prefix}":    f"Logistic ({label_prefix})",
            ...
            f"Adj_{label_prefix}_Note":    "Converged OK" if res.mle_retvals.get("converged") else "Not converged",
        }
        ...
        return {
            ...
            f"N_Adj_{label_prefix}":       len(df_model),
            f"Adj_{label_prefix}_Note":    f"Failed: {str(e)[:80]}",
        }
...
def _test_continuous(c_vals, nc_vals) -> Optional[Dict]:
    ...
    return {
        ...
        "Test_Adj_BMI": "N/A",
        "OR_Adj_BMI": np.nan, "OR_Adj_BMI_CI95": np.nan,
        "P_Adj_BMI":  np.nan, "N_Adj_BMI": np.nan,
...
def _test_binary(c_df, nc_df, col, age_col, bmi_col, include_bmi_model: bool = True) -> Optional[Dict]:
    ...
    if events_min >= MIN_EVENTS_LOGISTIC:
        ...
        if len(age_df) >= 6:
            ...
            result.update({
                "Test_Adj_Age": "Logistic (Age)", "OR_Adj_Age": np.nan, "OR_Adj_Age_CI95": np.nan,
                "P_Adj_Age": np.nan, "N_Adj_Age": len(age_df),
                "Adj_Age_Note": "Skipped: too few age-complete samples",
            })

        if include_bmi_model:
            bmi_only_df = combined[["outcome", "bmi", "carrier"]].dropna().copy()
            if len(bmi_only_df) >= 6:
                bmi_only_df["bmi_z"] = (bmi_only_df["bmi"] - bmi_only_df["bmi"].mean()) / bmi_only_df["bmi"].std()
                result.update(_run_logit(bmi_only_df, "outcome ~ carrier + bmi_z", "BMI"))
            else:
                result.update({
                    "Test_Adj_BMI": "Logistic (BMI)", "OR_Adj_BMI": np.nan, "OR_Adj_BMI_CI95": np.nan,
                    "P_Adj_BMI": np.nan, "N_Adj_BMI": len(bmi_only_df),
                    "Adj_BMI_Note": "Skipped: too few BMI-complete samples",
                })
            ...
            if len(bmi_df) >= 6:
                ...
                result.update(_run_logit(bmi_df, "outcome ~ carrier + age_z + bmi_z", "AgeBMI"))
            else:
                result.update({
                    "Test_Adj_AgeBMI": "Logistic (AgeBMI)", "OR_Adj_AgeBMI": np.nan, "OR_Adj_AgeBMI_CI95": np.nan,
                    "P_Adj_AgeBMI": np.nan, "N_Adj_AgeBMI": len(bmi_df),
                    "Adj_AgeBMI_Note": "Skipped: too few age+BMI-complete samples",
                })
        else:
            result.update({
                "Test_Adj_BMI": "N/A", "OR_Adj_BMI": np.nan, "OR_Adj_BMI_CI95": np.nan,
                "P_Adj_BMI": np.nan, "N_Adj_BMI": np.nan,
                "Adj_BMI_Note": "BMI-only model not requested for this analysis",
                "Test_Adj_AgeBMI": "N/A", "OR_Adj_AgeBMI": np.nan, "OR_Adj_AgeBMI_CI95": np.nan,
                "P_Adj_AgeBMI": np.nan, "N_Adj_AgeBMI": np.nan,
                "Adj_AgeBMI_Note": "Age+BMI model not requested for this analysis",
            })
    else:
        for sfx in ["Age", "BMI", "AgeBMI"]:
            result.update({
                f"Test_Adj_{sfx}": f"Logistic ({sfx})",
                f"OR_Adj_{sfx}": np.nan,
                f"OR_Adj_{sfx}_CI95": np.nan,
                f"P_Adj_{sfx}": np.nan,
                f"N_Adj_{sfx}": np.nan,
                f"Adj_{sfx}_Note": f"Skipped: too few events ({events_min} < {MIN_EVENTS_LOGISTIC})",
            })
    return result
...
def load_clinical_master(master_path: Path) -> pd.DataFrame:
    ...
    endo_pdl1 = master.get("clin_au_endo__FFPE_PDL1 POLAND+ BERGEN RESULTS", master.get("clin_au_endo__FFPE_PDL1 POLAND RESULTS", pd.Series(np.nan, index=master.index)))
    master["ENDO_PDL1_NUMERIC"]       = pd.to_numeric(endo_pdl1, errors="coerce")
...
def _run_clin_for_cohort(df_t: pd.DataFrame,
    ...
    for hap_id, hdf in df_t.groupby("Haplotype_ID"):
        ...
        for col, meta in var_dict.items():
            ...
            if meta["type"] == "continuous":
                ...
                # Fit age-only, BMI-only, and age+BMI adjusted models explicitly for binary outcomes
    ...
    for clin_col in res_df["Clinical_Var"].unique():
        ...
        if "P_Adj_BMI" in sub.columns and sub["P_Adj_BMI"].notna().any():
            sub = _apply_fdr(sub, "P_Adj_BMI", "FDR_Adj_BMI")
            sub["Nominal_Sig_Adj_BMI"] = sub["P_Adj_BMI"] < 0.05
            sub["FDR_Sig_Adj_BMI"]     = sub["FDR_Adj_BMI"] < FDR_THRESHOLD
        else:
            sub["FDR_Adj_BMI"] = np.nan
            sub["Nominal_Sig_Adj_BMI"] = False
            sub["FDR_Sig_Adj_BMI"]     = False
        if "P_Adj_AgeBMI" in sub.columns and sub["P_Adj_AgeBMI"].notna().any():
            sub = _apply_fdr(sub, "P_Adj_AgeBMI", "FDR_Adj_AgeBMI")
            sub["Nominal_Sig_Adj_AgeBMI"] = sub["P_Adj_AgeBMI"] < 0.05
            sub["FDR_Sig_Adj_AgeBMI"]     = sub["FDR_Adj_AgeBMI"] < FDR_THRESHOLD
        else:
            sub["FDR_Adj_AgeBMI"] = np.nan
            sub["Nominal_Sig_Adj_AgeBMI"] = False
            sub["FDR_Sig_Adj_AgeBMI"]     = False
        parts.append(sub)
...
def clinical_associations(merged: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for label, res in [("Breast", breast_res), ("Endometrial", endo_res)]:
        if res.empty:
            ...
            bmi_only_n = res["Nominal_Sig_Adj_BMI"].sum() if "Nominal_Sig_Adj_BMI" in res.columns else 0
            age_bmi_n  = res["Nominal_Sig_Adj_AgeBMI"].sum() if "Nominal_Sig_Adj_AgeBMI" in res.columns else 0
        ...
        if "age" in cox_df.columns and age_complete >= 6:
            if "bmi" in cox_df.columns and bmi_complete >= 6:
                ...
                if len(cox_df) >= 6:
                    ...
                    cox_df["bmi_z"] = (cox_df["bmi"] - cox_df["bmi"].mean()) / cox_df["bmi"].std()
                    note = (
    ...
    for cohort_label, cfg in SURVIVAL_COHORTS.items():
        ...
        for endpoint, ep_cfg in cfg["endpoints"].items():
            ...
            for hap_id, hdf in cdf.groupby("Haplotype_ID"):
                ...
                # Cox additive with explicit adjustment fallback
                cox_base = pd.DataFrame({"T": T, "E": E, "carrier": carrier})
                ...
                cox_df, cov, cox_adjustment, cox_adjustment_note = _prepare_cox_frame(cox_base, ["carrier"])
                p_cox  = np.nan
                hr     = np.nan
                hr_lo  = np.nan
                hr_hi  = np.nan
...
def _filter_significant_haplotype_rows(clin_res: pd.DataFrame, p_col: str | None = None) -> pd.DataFrame:
    """Keep only statistically significant rows for haplotype-clinical figures."""
...
def make_volcano(tvh: pd.DataFrame, out_dir: Path):
    ...
    for cohort, col in _COHORT_C.items():
        ...
        left_hits = []
        right_hits = []
        for _, row in sub[sub["Nominal_Sig"]].sort_values("P_Value").head(8).iterrows():
            point = (float(np.log2(row["OR"])), float(-np.log10(row["P_Value"])), row["Haplotype_ID"])
            if point[0] < 0:
...
def make_heatmap(clin_res: pd.DataFrame, cohort_label: str,
    ...
    fig_w = max(14, n_vars * col_w * 2.35 + 4.5)   # extra width keeps clinical labels readable
    ...
    ax_p.tick_params(axis="x", labelsize=x_fs)
    ax_p.tick_params(axis="y", labelsize=8.5)
    _set_wrapped_ticklabels(ax_p, axis="x", width=18, max_lines=3, rotation=0, ha="center", fontsize=x_fs)
    ...
    ax_es.set_xlabel("")
    ax_es.set_ylabel("")
    ax_es.tick_params(axis="x", labelsize=x_fs)
    _set_wrapped_ticklabels(ax_es, axis="x", width=18, max_lines=3, rotation=0, ha="center", fontsize=x_fs)
...
def make_risk_forest(risk_res: pd.DataFrame, out_path: Path):
    ...
    fig, ax = plt.subplots(figsize=(10.5, max(5, n * 0.5 + 1.5)))
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        ax.text(1.02, i, ann, transform=ax.get_yaxis_transform(), va="center", ha="left",
...
def make_main_text_heatmaps(b_clin: pd.DataFrame, e_clin: pd.DataFrame, out_dir: Path):
    ...
    for clin_res, cohort_label in [(b_clin, 'Breast'), (e_clin, 'Endometrial')]:
        if clin_res.empty:
            continue
        significant = _filter_significant_haplotype_rows(clin_res)
        if significant.empty:
            continue
        focused = _select_haplotype_heatmap_focus_rows(significant, 'P_Unadj', max_rows=10)
        make_heatmap(
            focused,
            cohort_label,
            'P_Unadj',
...
def make_haplotype_composition_plots(tvh: pd.DataFrame, merged: pd.DataFrame, out_dir: Path):
    ...
    status_short = {'Wild-type': 'WT', 'Heterozygous': 'Het', 'Homozygous': 'Hom'}
        ...
        for idx, (_, row) in enumerate(chosen.iterrows()):
            ...
            for status in status_order:
                ...
                ax.bar(
                    ...
                    vals,
                    bottom=bottoms,
                    color=HAPLOTYPE_STATUS_COLORS[status],
                    edgecolor='#4A4A4A',
                    linewidth=1.0,
                    width=0.58,
...
def main():
    ...
    if not b_clin.empty and "P_Unadj" in b_clin.columns and b_clin["P_Unadj"].notna().any():
        ...
        make_heatmap(
            ...
            significant_dir / "18_Haplo_Heatmap_Breast.png",
        )
    if not e_clin.empty and "P_Unadj" in e_clin.columns and e_clin["P_Unadj"].notna().any():
        ...
        make_heatmap(
            ...
            significant_dir / "18_Haplo_Heatmap_Endometrial.png",
        )
    if not b_clin.empty and "P_Adj_Age" in b_clin.columns and b_clin["P_Adj_Age"].notna().any():
        ...
        make_heatmap(
            ...
            significant_dir / "18_Haplo_Heatmap_Breast_AgeAdj.png",
        )
    if not e_clin.empty and "P_Adj_Age" in e_clin.columns and e_clin["P_Adj_Age"].notna().any():
        ...
        make_heatmap(
            ...
            significant_dir / "18_Haplo_Heatmap_Endometrial_AgeAdj.png",
        )
    if not b_clin.empty and "P_Adj_BMI" in b_clin.columns and b_clin["P_Adj_BMI"].notna().any():
        ...
        make_heatmap(
            ...
            significant_dir / "18_Haplo_Heatmap_Breast_BMIAdj.png",
        )
    if not e_clin.empty and "P_Adj_BMI" in e_clin.columns and e_clin["P_Adj_BMI"].notna().any():
        ...
        make_heatmap(
            ...
            significant_dir / "18_Haplo_Heatmap_Endometrial_BMIAdj.png",
        )
    if not b_clin.empty and "P_Adj_AgeBMI" in b_clin.columns and b_clin["P_Adj_AgeBMI"].notna().any():
        ...
        make_heatmap(
            ...
            significant_dir / "18_Haplo_Heatmap_Breast_AgeBMIAdj.png",
        )
    if not e_clin.empty and "P_Adj_AgeBMI" in e_clin.columns and e_clin["P_Adj_AgeBMI"].notna().any():
        ...
        make_heatmap(
            ...
            significant_dir / "18_Haplo_Heatmap_Endometrial_AgeBMIAdj.png",
        )
