...
def _plot_pca_redesign(full_qc_df: pd.DataFrame, out_path: str, pass_only: bool = False):
    ...
    if handles:
        ...
        ax.legend(handles[:n], [_display_group(lbl) for lbl in labels[:n]], title="Cohort group", bbox_to_anchor=(1.02, 1), loc="upper left", frameon=False)
...
def _plot_amplicon_risk_heatmap_redesign(cohort_dfs: dict, main_dir: str):
    ...
    sns.heatmap(plot_df, cmap=sns.color_palette(["#F4F4F4", "#F2C14E", "#D95D39", "#7F0000"], as_cmap=True), vmin=0, vmax=max(5.0, float(np.nanmax(plot_df.values))), linewidths=0.15, linecolor="#FFFFFF", cbar_kws={"label": "% of samples below 50×"}, ax=ax)
...
def _plot_gap_and_audit_panels_redesign(cohort_dfs: dict, full_qc_df: pd.DataFrame, audit_dir: str):
    ...
    if not gap_df.empty:
        ...
        ax.set_ylabel("Amplicons below 50×")
        ax.set_xlabel("")
        ax.grid(axis="y", linestyle=":", alpha=0.3)
        plt.tight_layout()
    ...
    if not full_qc_df.empty:
        ...
        fig.legend(handles, ["Pass", "Fail"], title="QC status", loc="upper center", ncol=2, frameon=False, bbox_to_anchor=(0.5, 1.02))
        plt.tight_layout()
    ...
    if not worst_df.empty:
        ...
        ax.set_ylabel("Minimum amplicon depth (×)")
        ax.set_xlabel("")
        ax.set_xticks(np.arange(len(FIXED_GROUP_ORDER)))
        ax.set_xticklabels([_display_group(g).replace(" ", "\n") for g in FIXED_GROUP_ORDER], fontsize=8.5)
        ax.grid(axis="y", linestyle=":", alpha=0.3)
...
def _plot_systemic_failure_redesign(cohort_dfs: dict, audit_dir: str, label_col: str, out_name: str, title: str):
    ...
    axes[-1].set_xlabel("Failed samples below 50×")
...
def _plot_global_depth_hist_redesign(all_cov: pd.DataFrame, audit_dir: str):
    ...
    ax.set_xlabel(f"Coverage depth (×), clipped at 99.5th percentile = {upper:.0f}×")
