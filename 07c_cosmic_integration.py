...
This cohort uses unpaired samples — healthy tissue and tumour tissue are
derived from different patients. Somatic/germline classification requires
matched tumour-normal pairs and is therefore NOT performed here.
Variants are instead compared between tissue types (Tumour vs. Healthy)
at the population level across each cohort.
"""
...
def summarise_tissue_impact(df, tis_c, imp_c):
    """
    Summarise variant impact distribution by tissue type (Tumour vs. Healthy).
    Appropriate for unpaired study designs where somatic/germline calling
    is not possible.

    Returns:
        DataFrame of impact counts per tissue type
    """
    tissue_impact = (
    ...
    # Reorder impact columns if present
...
def analyze_cosmic_context():
    ...
    print(f"✓ Found {len(census_variants)} variants in "
          f"{census_variants[sym_c].nunique()} COSMIC census genes")
    ...
    print("STEP 3: Variant Impact by Tissue Type (Tumour vs. Healthy)")
    print("─"*70)
    print("  NOTE: Somatic/germline classification is not performed.")
    print("  Samples are unpaired; tumour and healthy tissue are from")
    print("  different patients. Variants are compared at the population")
    print("  level by tissue type.\n")
    ...
    if 'Breast' in cancer_specific.columns and 'Endometrium' in cancer_specific.columns:
        ...
        print(f"✓ Cancer specificity (tumour samples only):")
...
def generate_cosmic_plots(df, summary_by_gene, hotspots, cancer_specific,
                          tissue_impact, sym_c, tis_c, coh_c, imp_c='IMPACT'):
    """Generate comprehensive COSMIC analysis visualisations."""
    ...
    # Plot 3: Variant Impact by Tissue Type (replaces somatic/germline)
