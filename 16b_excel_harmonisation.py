...
# -*- coding: utf-8 -*-
"""
16b_excel_harmonisation.py  (Option B: lossless merge + semantic harmonisation)  v4
...
def apply_semantic_transforms(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "canon__bmi" in out.columns:
        def _parse_bmi(x) -> Optional[float]:
            if pd.isna(x): return None
            s = str(x).strip().upper()
            if s in ("X", "SOBREPESO", "OBESIDAD", ""): return None
            # extract first number (handles '53.5 kg', '60 kg; 168 cm')
            m = re.search(r"(\d+(?:[.,]\d+)?)", s)
            if m:
                v = float(m.group(1).replace(",", "."))
                # sanity: reject weights (> 80 likely kg not BMI) and implausible values
                if 10.0 < v < 80.0:
                    return v
            return None
        out["canon__bmi"] = out["canon__bmi"].map(_parse_bmi)
    ...
    if "canon__ki67" in out.columns:
        def _parse_ki67(x) -> Optional[float]:
            if pd.isna(x): return None
            s = str(x).strip().upper()
            if s in ("X", ""): return None
            # range string: take midpoint
            m_range = re.match(r"(\d+(?:\.\d+)?)\s*[-–]\s*(\d+(?:\.\d+)?)%?", s)
            if m_range:
                return (float(m_range.group(1)) + float(m_range.group(2))) / 2
            m_gt = re.match(r"[>≥]\s*(\d+(?:\.\d+)?)%?", s)
            if m_gt:
                return float(m_gt.group(1))
            m_num = re.search(r"(\d+(?:[.,]\d+)?)", s)
            if m_num:
                return float(m_num.group(1).replace(",", "."))
            return None

        raw_ki67 = out["canon__ki67"].map(_parse_ki67)

        # Convert: DCS values are fractions (0-1); endo values are percentages (0-100)
        # Heuristic: if raw value <= 1.0 it is a fraction from DCS -> multiply by 100
        out["canon__ki67_pct"] = raw_ki67.apply(
            lambda v: round(v * 100, 1) if (v is not None and v <= 1.0) else v
        )
        # Keep original coalesced value as-is in canon__ki67 for provenance
        out["canon__ki67"] = raw_ki67
    ...
    if "canon__p53_ihc" in out.columns:
        raw_p53 = pd.to_numeric(out["canon__p53_ihc"], errors="coerce")
        out["canon__p53_ihc_raw"] = raw_p53
        out["canon__p53_status"] = raw_p53.apply(
            lambda v: "Aberrant" if pd.notna(v) and v >= 0.10
            else ("Normal" if pd.notna(v) else pd.NA)
        )
