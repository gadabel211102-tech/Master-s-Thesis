...
from objective2_rna_qc import RNA_QC_COLUMNS, attach_rna_qc, bam_validation, plot_rna_qc
from objective2_gene_exports import export_panel_gene_reports
...
def sx(v):
    t=s(v)
    for pat,fmt in [(r"^(SNP_(?:AT|EN|MN|MT-T)_\d+)$",lambda m:m.group(1).upper()),(r"^SNP_DNA_(EN|MN|AT)_(\d+)$",lambda m:f"SNP_{m.group(1).upper()}_{m.group(2)}"),(r"^SNP_DNA_MT[-_]T_(\d+)$",lambda m:f"SNP_MT-T_{m.group(1)}"),(r"^DNA_SNP_(EN|MN)_(\d+)",lambda m:f"SNP_{m.group(1).upper()}_{m.group(2)}"),(r"^DNA_EN_(\d+)",lambda m:f"SNP_EN_{m.group(1)}"),(r"^DNA_MN_(\d+)",lambda m:f"SNP_MN_{m.group(1)}"),(r"^DNA_SNP_AT_(\d+)",lambda m:f"SNP_AT_{m.group(1)}"),(r"^DNA_(?:SNP_)?MT[-_]T_(\d+)",lambda m:f"SNP_MT-T_{m.group(1)}"),(r"^RNA_SNP_(AT|EN|MN|MT-T)_(\d+)",lambda m:f"SNP_{m.group(1).upper()}_{m.group(2)}"),(r"^SNP_RNA_(AT|EN|MN|MT-T)_(\d+)$",lambda m:f"SNP_{m.group(1).upper()}_{m.group(2)}"),(r"^RNA_(AT|EN|MN)_(\d+)",lambda m:f"SNP_{m.group(1).upper()}_{m.group(2)}"),(r"^RNA_MT[-_]T_(\d+)",lambda m:f"SNP_MT-T_{m.group(1)}")]:
...
def plot_dist(df,out,controls):
    p=df.copy();
    if not controls: p=p[p["analysis_role"]=="primary_tumour"].copy()
    if p.empty: return
    fig,ax=plt.subplots(2,3,figsize=(15,8),constrained_layout=True); order=[g for g in ["Breast_Tumour","Endometrial_Tumour","Breast_Normal","Endometrial_Normal"] if g in p["analysis_group"].unique()]
    for a,e in zip(ax.flat,["G1","G2","G3","G3b","G4","GSDMB"]):
        t=p[[e,"analysis_group"]].dropna().copy();
        if t.empty: a.set_visible(False); continue
        if sns is not None:
            sns.boxplot(data=t,x="analysis_group",y=e,order=order,ax=a,color="#d9e8f5")
            sns.stripplot(data=t,x="analysis_group",y=e,order=order,ax=a,color="#2f5d80",size=3,alpha=0.7)
        else:
            labels=[]; groups=[]
            for lab in order:
                vals=pd.to_numeric(t.loc[t["analysis_group"].astype(str)==lab,e],errors="coerce").dropna().values
                if len(vals):
            ...
            if groups:
                ...
                for i,vals in enumerate(groups, start=1):
                    a.scatter(np.full(len(vals), i), vals, color="#2f5d80", s=8, alpha=0.7)
        a.set_title(e); a.set_xlabel(""); a.tick_params(axis="x",rotation=25)
    fig.suptitle("Objective 2: GSDMB isoform and total-expression distributions",fontsize=14,fontweight="bold"); fig.savefig(out/"20_Isoform_Distributions.png",dpi=300,bbox_inches="tight"); plt.close(fig)
...
def heat(df,row,col,val,out,title):
    ...
    if p.empty: return
    fig,a=plt.subplots(figsize=(12,max(4,0.4*len(p.index))))
    if sns is not None:
        sns.heatmap(p,cmap="coolwarm",center=0,linewidths=0.5,linecolor="white",ax=a)
    else:
        im=a.imshow(p.values, aspect="auto", cmap="coolwarm")
        a.set_xticks(range(len(p.columns))); a.set_xticklabels(p.columns, rotation=45, ha="right")
        a.set_yticks(range(len(p.index))); a.set_yticklabels(p.index)
        fig.colorbar(im, ax=a, fraction=0.03, pad=0.02)
    a.set_title(title,fontsize=13,fontweight="bold"); fig.savefig(out,dpi=300,bbox_inches="tight")
    plt.close(fig)
