...
def script19b_defaults() -> dict[str, object]:
    ...
    return {
        ...
        "machine_qc_xlsx": paths["objective2_rna_machine_qc_workbook"],
...
def script26_defaults() -> dict[str, object]:
    """Return the canonical configuration bundle for SNP functional interpretation."""
    paths = get_paths()
    thresholds = get_thresholds()
    return {
        "annot": paths["annotated_report"],
        "master": paths["harmonised_master"],
        "out_dir": paths["snp_functional_interpretation_dir"],
        "curated_evidence": paths["curated_snp_functional_evidence"],
        "stage08": paths["variant_landscape_dir"] / "GSDMB_Global_Variant_Landscape.xlsx",
        "stage09": paths["gsdmb_landscape_dir"] / "GSDMB_Target_Gene_Variant_Landscape_Split.xlsx",
        "stage12": paths["snp_enrichment_dir"] / "GSDMB_SNP_Enrichment_Results.xlsx",
        "stage17": paths["snp_clinical_dir"] / "GSDMB_SNP_Clinical_Association_Results.xlsx",
        "stage20": paths["isoform_expression_dir"] / "GSDMB_Objective2_Isoform_Results.xlsx",
        "stage20b": paths["results_dir"] / "20b_panel_gene_expression" / "20b_Panel_Gene_Expression.xlsx",
        "stage22": paths["results_dir"] / "22_haplotype_first_interpretation" / "22_Haplotype_First_Interpretation.xlsx",
        "stage24": paths["results_dir"] / "24_external_cross_validation" / "24_External_Cross_Validation.xlsx",
        "fdr_threshold": thresholds["fdr_threshold"],
        "grouping": get_grouping(),
    }

def script24_defaults() -> dict[str, object]:
    """Return the canonical configuration bundle for external cross-validation."""
    paths = get_paths()
    return {
        "haplotype_results": paths["haplotype_results"],
        "snp_results": paths["snp_enrichment_dir"] / "GSDMB_SNP_Enrichment_Results.xlsx",
        "study_sample_metadata": paths["haplotype_phased_dir"] / "sample_metadata.tsv",
        "study_female_samples": paths["haplotype_phased_dir"] / "female_by_design_study_samples.tsv",
        "reference_population": "EUR",
        "age_min": 45,
        "age_max": 65,
        "out_dir": paths.get("external_validation_dir", paths["results_dir"] / "24_external_cross_validation"),
    }
